...
export class WCFComponent implements OnInit {
  ...
  getUsers() {
    this.userList = [];
    this.http.get("https://localhost:44368/api/Values").subscribe((data: any) => {
      this.userList = data
    });
  }
  ...
  postUserFormData() {
    ...
    this.http.post("https://localhost:44368/api/Values",this.userForm.value).subscribe(data=>{
      this.getUsers();
    },response=>{
  ...
  updateUserFormData() {
    ...
    this.http.put("https://localhost:44368/api/Values/"+this.userForm.value.userId,this.userForm.value).subscribe(data=>{
      this.getUsers();
    },response=>{
      if(response.status==200)
      {
  ...
  //Delete UserData
  deleteUser(id:any) {
    this.http.delete("https://localhost:44368/api/Values/"+id).subscribe(data=>{},
      response=>{
        if(response.status==200){
